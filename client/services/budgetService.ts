import { api } from './api';
import { Budget, CreateBudgetDTO, UpdateBudgetDTO } from '../types/budget';

const STORAGE_KEY = 'budgets';

export const budgetService = {
  async getAll(): Promise<Budget[]> {
    try {
      return await api.get<Budget[]>('/budgets');
    } catch {
      return this.getAllLocal();
    }
  },

  async create(data: CreateBudgetDTO): Promise<Budget> {
    try {
      return await api.post<Budget>('/budgets', data);
    } catch {
      return this.createLocal(data);
    }
  },

  async update(id: string, data: UpdateBudgetDTO): Promise<Budget> {
    try {
      return await api.put<Budget>(`/budgets/${id}`, data);
    } catch {
      return this.updateLocal(id, data);
    }
  },

  async delete(id: string): Promise<void> {
    try {
      await api.delete(`/budgets/${id}`);
    } catch {
      this.deleteLocal(id);
    }
  },

  // Local storage fallbacks
  getAllLocal(): Budget[] {
    const data = localStorage.getItem(STORAGE_KEY);
    return data ? JSON.parse(data) : [];
  },

  createLocal(data: CreateBudgetDTO): Budget {
    const budgets = this.getAllLocal();
    const budget: Budget = {
      id: Math.random().toString(36).substr(2, 9),
      userId: 'local',
      ...data,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    };
    budgets.push(budget);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(budgets));
    return budget;
  },

  updateLocal(id: string, data: UpdateBudgetDTO): Budget {
    const budgets = this.getAllLocal();
    const index = budgets.findIndex(b => b.id === id);
    if (index === -1) throw new Error('Budget not found');
    
    budgets[index] = {
      ...budgets[index],
      ...data,
      updatedAt: new Date().toISOString(),
    };
    localStorage.setItem(STORAGE_KEY, JSON.stringify(budgets));
    return budgets[index];
  },

  deleteLocal(id: string): void {
    const budgets = this.getAllLocal();
    const filtered = budgets.filter(b => b.id !== id);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(filtered));
  },
};
